...
class PlateRecognitionApp:
    ...
    def configure_styles(self):
        ...
        self.style.map(
            "TNotebook.Tab",
            focuscolor=[("selected", self.tab_selected)],
    ...
    def create_widgets(self):
        ...
        btn_load = ttk.Button(
            control_frame, text="Carregar Imagem", command=self.load_image
        )

        btn_load.pack(side=tk.LEFT, padx=5, pady=5)

        btn_process = ttk.Button(
            control_frame, text="Processar Placa", command=self.process_plate
        )
    ...
    def create_char_frames(self, parent):
        ...
        for i in range(7):  # 7 caracteres na placa Mercosul
            frame = tk.Frame(parent, bg="#1a2232", bd=2, relief=tk.GROOVE)
    ...
    def process_plate(self):
        ...
        try:
            ...
            self.plate_text, self.artifacts = recognize_plate(
                self.image_path, "characters", collect_artifacts=True
            )
            ...
            if self.plate_text:
                ...
                self.result_label.config(
                    text="Não foi possível reconhecer a placa", fg="#e74c3c"
                )
    ...
    def clear_pipeline(self):
        ...
        for idx, frame_data in enumerate(self.char_frames):
            frame_data["img_label"].config(image="")

            frame_data["img_label"].image = None

            frame_data["text_label"].config(text=f"Char {idx}")
