...
def save_artifacts(artifacts: PipelineArtifacts, out_dir: str) -> None:
    ...
    def _save(name: str, img: Optional[np.ndarray]):
        if img is not None: cv2.imwrite(os.path.join(out_dir, name), img)
    ...
    for i, seg in enumerate(artifacts.segmented):
        ...
        if seg.proc50 is not None: _save(f"char_{i:02d}_proc50.png", seg.proc50)

def show_debug_windows(artifacts: PipelineArtifacts) -> None:
    """Exibe janelas OpenCV das etapas (opcional para debug)."""
    if artifacts.original_bgr is not None: cv2.imshow("00 Original", artifacts.original_bgr)
    if artifacts.gray is not None: cv2.imshow("01 Gray", artifacts.gray)
    if artifacts.binary_inv_otsu is not None: cv2.imshow("02 Binary INV Otsu", artifacts.binary_inv_otsu)
    if artifacts.eroded is not None: cv2.imshow("03 Eroded", artifacts.eroded)
    for i, seg in enumerate(artifacts.segmented):
        cv2.imshow(f"char {i:02d} - raw", seg.raw_crop)
        cv2.imshow(f"char {i:02d} - std", seg.standardized)
        if seg.proc50 is not None: cv2.imshow(f"char {i:02d} - proc50", seg.proc50)
