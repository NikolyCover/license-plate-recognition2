...
def recognize_plate(image_path: str, models_path: str,
                    ...
                    collect_artifacts: bool = True) -> Tuple[str, Optional[PipelineArtifacts]]:
    ...
    arts = PipelineArtifacts(original_bgr=img.copy() if collect_artifacts else None,
    ...
    if collect_artifacts:
        for (x,y,w,h), std in zip(boxes, chars_std):
            ...
            arts.segmented.append(SegmentArtifact(bbox_x=x, raw_crop=raw, standardized=std, proc50=proc50, contours=cnts))

    if not chars_std: return "", arts
    models = load_models(models_path)
    if not models: return "", arts
...
if __name__ == "__main__":
    text, arts = recognize_plate("mock/PLATE_1.png", "characters", collect_artifacts=True)
